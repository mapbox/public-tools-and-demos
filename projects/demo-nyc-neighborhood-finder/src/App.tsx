...
function App() {
  ...
  const [profileNeighborhood, setProfileNeighborhood] = useState<RecommendedNeighborhood | null>(null)
  const [errorDismissed, setErrorDismissed] = useState(false)
  const [input, setInput] = useState('')

  const { index: neighborhoodIndex, features: neighborhoodFeatures, getBounds } = useNeighborhoodData()
  const { messages, sendMessage, status, error, clearError } = useChat({ api: '/api/chat' })
  ...
  const { neighborhoodsByMessageId, recommendedNeighborhoods } = useMemo(() => {
    ...
    for (const msg of messages) {
      ...
      const invPart = msg.parts.find(p =>
      ...
      const rawNeighborhoods = (parsed as { neighborhoods: unknown }).neighborhoods
      const neighborhoods = typeof rawNeighborhoods === 'string'
        ? JSON.parse(rawNeighborhoods) as { name: string; borough: string; reason?: string; vibe?: string }[]
        : rawNeighborhoods as { name: string; borough: string; reason?: string; vibe?: string }[]
      ...
      const enriched = neighborhoods.map(n => {
        ...
        const color = NEIGHBORHOOD_COLORS[colorIdx++ % NEIGHBORHOOD_COLORS.length]
        const enrichedN: RecommendedNeighborhood = {
          ...n, color,
          reason: n.reason ?? n.vibe ?? (n as Record<string,unknown>).description as string ?? '',
    ...
    return { neighborhoodsByMessageId: byId, recommendedNeighborhoods: Array.from(allSeen.values()) }
  }, [messages, neighborhoodIndex])

  // Only fit bounds to the most-recently-recommended set
  const boundsToFit = useMemo(() => {
    const last = [...messages].reverse().find(msg =>
      msg.role === 'assistant' && msg.parts.some(p => p.type === 'tool-show_neighborhoods')
    )
    if (!last) return null
    const invPart = last.parts.find(p => p.type === 'tool-show_neighborhoods' && (p.state === 'input-available' || p.state === 'output-available'))
    if (!invPart || invPart.type !== 'tool-show_neighborhoods' || !invPart.input) return null
    const raw = invPart.input
    const parsed = typeof raw === 'string' ? JSON.parse(raw) : raw as { neighborhoods: unknown }
    const rawNeighborhoods = (parsed as { neighborhoods: unknown }).neighborhoods
    const neighborhoods = typeof rawNeighborhoods === 'string'
  ...
  return (
    <div className="app-layout">
      ...
      {!errorDismissed && error && (
        <Toast message={error.message} onDismiss={() => { setErrorDismissed(true); clearError() }} />
      )}
    </div>
  )
}
