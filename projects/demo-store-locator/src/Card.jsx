...
const Card = ({ feature, onClick, activeFeature }) => {
  ...
  const isActiveFeature =
    feature.properties.name == activeFeature?.properties.name ? true : false

  let isMarkerHovered

  if (hoveredFeature) {
    isMarkerHovered =
      feature.properties.address == hoveredFeature.properties.address
        ? true
        : false
  }
  ...
  return (
    <div
      className={`rounded-md cursor-pointer p-4 
          ${
            isActiveFeature
              ? 'bg-tintgreen border-maroon border-2'
              : 'hover:bg-slate-100'
          }
